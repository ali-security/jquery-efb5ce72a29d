...
test("jQuery.extend( true, ... ) Object.prototype pollution", function() {
	...
	ok( !( "devMode3" in jQuery.extend( {}, JSON.parse( "{\"__proto__\": {\"devMode3\": true}}" ) ) ),
		"__proto__ not copied in a shallow extend" );
});
...
if ( jQuery.support.createHTMLDocument ) {
	asyncTest( "jQuery.parseHTML", function() {
		...
		img = jQuery.parseHTML( "<img src=x onerror='parseHTMLError = true'>" )[ 0 ];

		notStrictEqual( img.ownerDocument, document,
			"The markup is parsed in an inert document, not the live one" );
